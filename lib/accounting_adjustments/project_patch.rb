            ...
            def get_accounting_adjustments(start_date = nil, end_date = nil)
                if accounting_adjustment_enabled
                    if start_date.present? and end_date.present?
                        issues.joins("LEFT JOIN custom_values AS amount ON amount.custom_field_id = ? AND amount.customized_id = issues.id LEFT JOIN custom_values AS type ON type.custom_field_id = ? AND type.customized_id = issues.id LEFT JOIN custom_values AS date ON date.custom_field_id = ? AND date.customized_id = issues.id").where("tracker_id = ? AND type.value = ? AND ((MONTH(date.value) = 12 AND DAY(date.value) = 31) OR (MONTH(date.value) = 1 AND DAY(date.value) = 1)) AND YEAR(date.value) BETWEEN ? AND ?", Setting.plugin_redmine_accounting_adjustments['amount_field'], Setting.plugin_redmine_accounting_adjustments['type_field'], Setting.plugin_redmine_accounting_adjustments['date_field'], Setting.plugin_redmine_accounting_adjustments['tracker'], Setting.plugin_redmine_accounting_adjustments['type_value'], start_date.year, end_date.year) #.select("amount.value AS amount")
                    elsif start_date.present?
                        issues.joins("LEFT JOIN custom_values AS amount ON amount.custom_field_id = ? AND amount.customized_id = issues.id LEFT JOIN custom_values AS type ON type.custom_field_id = ? AND type.customized_id = issues.id LEFT JOIN custom_values AS date ON date.custom_field_id = ? AND date.customized_id = issues.id").where("tracker_id = ? AND type.value = ? AND ((MONTH(date.value) = 12 AND DAY(date.value) = 31) OR (MONTH(date.value) = 1 AND DAY(date.value) = 1)) AND YEAR(date.value) >= ?", Setting.plugin_redmine_accounting_adjustments['amount_field'], Setting.plugin_redmine_accounting_adjustments['type_field'], Setting.plugin_redmine_accounting_adjustments['date_field'], Setting.plugin_redmine_accounting_adjustments['tracker'], Setting.plugin_redmine_accounting_adjustments['type_value'], start_date.year) #.select("amount.value AS amount")
                    elsif end_date.present?
                        issues.joins("LEFT JOIN custom_values AS amount ON amount.custom_field_id = ? AND amount.customized_id = issues.id LEFT JOIN custom_values AS type ON type.custom_field_id = ? AND type.customized_id = issues.id LEFT JOIN custom_values AS date ON date.custom_field_id = ? AND date.customized_id = issues.id").where("tracker_id = ? AND type.value = ? AND ((MONTH(date.value) = 12 AND DAY(date.value) = 31) OR (MONTH(date.value) = 1 AND DAY(date.value) = 1)) AND YEAR(date.value) <= ?", Setting.plugin_redmine_accounting_adjustments['amount_field'], Setting.plugin_redmine_accounting_adjustments['type_field'], Setting.plugin_redmine_accounting_adjustments['date_field'], Setting.plugin_redmine_accounting_adjustments['tracker'], Setting.plugin_redmine_accounting_adjustments['type_value'], end_date.year) #.select("amount.value AS amount")
                    else
                        issues.joins("LEFT JOIN custom_values AS amount ON amount.custom_field_id = ? AND amount.customized_id = issues.id LEFT JOIN custom_values AS type ON type.custom_field_id = ? AND type.customized_id = issues.id LEFT JOIN custom_values AS date ON date.custom_field_id = ? AND date.customized_id = issues.id").where("tracker_id = ? AND type.value = ? AND ((MONTH(date.value) = 12 AND DAY(date.value) = 31) OR (MONTH(date.value) = 1 AND DAY(date.value) = 1))", Setting.plugin_redmine_accounting_adjustments['amount_field'], Setting.plugin_redmine_accounting_adjustments['type_field'], Setting.plugin_redmine_accounting_adjustments['date_field'], Setting.plugin_redmine_accounting_adjustments['tracker'], Setting.plugin_redmine_accounting_adjustments['type_value']) #.select("amount.value AS amount")
                    end
                else
                    Issue.none
                end
